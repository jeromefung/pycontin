...
def _binary_search(target_value, alpha0, inv_input, int_res,
                   soln0, converge_radius = 0.05):
    prob1s = np.array([0.]) # remove this later
    solns = []
    working_alphas = np.array([alpha0]) # remove later

    working_alpha = alpha0

    while prob1s.max() <= target_value:
        working_alpha *= 10
        soln = _solve_fixed_alpha(inv_input, working_alpha, intermed_res = 
                                  int_res)
